 ...
 * @brief 数字传感器监视对象
 *
 * 用于监视数字信号引脚（如水浸传感器）的高低电平变化，
 * 内置信号抖动过滤（debounce）机制，状态变化时触发回调。
...
/**
 * 数字传感器监视类
 * 通过 digitalRead 轮询引脚电平，检测状态变化并过滤抖动。
 * 继承 IInterruptComponent 以支持流程中断检测。
 */
class Sensor : public IInterruptComponent {

public:
  /**
   * 状态变化回调函数类型
   * @param state 当前状态（true=HIGH, false=LOW）
   * @param timestamp 状态变化时间戳（毫秒）
   ...
   */
  typedef void (*ChangeHandler)(bool state, unsigned long timestamp,
                                bool lastState, unsigned long lastTimestamp,
                                unsigned long duration, Sensor *sensor,
                                void *context);
